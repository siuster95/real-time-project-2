  ...
  io.on('connection', (sock) => {
    ...
    socket.on('updateFromclient', (data) => {
      if (data.hash !== undefined) {
        let roomToupdate = characters[data.roomNumber];
    ...
    socket.on('disconnect', () => {
      let Rnd = socket.roomNumber;
      let room = characters[Rnd];
      const keys = Object.keys(room);
      let hashout = "";
      for (let i = 0; i < keys.length; i++) {
        let object = room[keys[i]];
...
const changeState = (state, circle) => {
  ...
  switch(state) {
    case 0:
      let rN = circle.roomNumber;
      let specificRoom0 = characters[rN];
      specificRoom0["state"] = 0;
      io.to(`room${circle.roomNumber}`).emit("someoneHasball", {"circle":circle,"state":state});
      break;
    case 1:
    let rN1 = circle.roomNumber;
    let specificRoom1 = characters[rN1];
...
const stateStatus = (circle) => {
  let rNs = circle.roomNumber;
  let specificRoomS = characters[rNs];
  let stateOut = specificRoomS["state"];
  return stateOut;
}
